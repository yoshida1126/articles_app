...
RSpec.describe 'ArticleComments', type: :system, js: true do
  ...
  describe '#create' do
    ...
    context 'as a logged in user(input correct article comment)' do
      ...
      it 'コメントの投稿に成功すること' do
        expect(page).to have_field('article_comment[comment]', wait: 10)
  ...
  describe '#update' do
    ...
    context 'as a logged in user(input correct article comment)' do
      ...
      it 'コメントを編集できること' do
        fill_in('article_comment[comment]', match: :first, with: 'Edit Article Comment', wait: 10)
        click_button '編集'
        expect(page).to have_content('Edit Article Comment', wait: 10)
      end
    ...
    context 'as a logged in user(input wrong article comment)' do
      ...
      it 'コメントフォームが空だと編集できないこと' do
        fill_in('article_comment[comment]', match: :first, with: '', wait: 10)
        click_button '編集'
        expect(page).to have_selector('div.alert-danger', wait: 10)
      end
    end
  ...
  describe '#destroy' do
    ...
    context 'as a logged in user' do
      ...
      it '削除したコメントがコメント欄にないこと' do
        expect(page).to have_no_content('Article Comment', wait: 10)
      end
    end
  end
end