...
RSpec.describe LikeRateLimiterService, type: :service do

    describe '#allowed?' do
        let(:user) { FactoryBot.create(:user) }
        let(:article) { FactoryBot.create(:article) }
        let(:service) { described_class.new(user: user, article: article ) }
    ...
    describe '#record_like_time' do
        ...
        it 'いいねした時間がRedisに記録されること' do
            Timecop.freeze(Time.now) do
                ...
                service.record_like_time
                stored_time = $redis.get("user:#{user.id}:like:#{article.id}").to_i
