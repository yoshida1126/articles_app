...
- **記事・コメント入力時にリアルタイム文字数カウントを表示（Stimulusを利用）** など、操作中の負担を減らす工夫をしています。  
...
- **投稿機能**
  - 記事の投稿・編集機能を実装。現在は基本的なCRUD操作に加えて、下書き保存機能をサポートしており、ユーザーは編集途中の記事を下書きとして保存できます。今後は、**非同期での自動保存（オートセーブ）**に対応し、編集中の体験をさらに快適にする予定です。
...
- **検索機能（ransack）**
  ...
  - 検索キーワードの先頭に#(ハッシュタグ)をつけることにより**タグでの検索**も可能
